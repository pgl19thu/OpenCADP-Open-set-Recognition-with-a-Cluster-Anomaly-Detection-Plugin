...
def generate_unknown_class(num_unknown):
    ...
    unknown_classes = ['WebDDoS', 'UDPLag']
...
if __name__ == '__main__':
    ...
    dict_class = {'BENIGN': 0, 'DrDoS_DNS': 1, 'DrDoS_LDAP': 2, 'DrDoS_MSSQL': 3, 'DrDoS_NTP': 4,
                  'DrDoS_NetBIOS': 5, 'DrDoS_SNMP': 6, 'DrDoS_SSDP': 7,
                  'DrDoS_UDP': 8, 'Syn': 9, 'TFTP': 10,
                  'UDPLag': 11, 'WebDDoS': 12}
    ...
    for times1 in range(5):
        ...
        for num_unknowns in range(1, 10):
            print("设置项目工作路径")
            ...
            if not isExists:
                ...
                noise_uniform = np.random.random((500, 66))
            ...
            pred_best_methods = []
            for method in ['softmax', 'openmax', 'doc', 'center_ids', 'center_ii', 'cvae_evt', 'crosr']:
                # 'softmax', 'cvae_evt', 'openmax', 'crosr', 'center_ii', 'center_ids','doc'
                ...
                if is_adv:
                    ...
                    if len(real_target) != 0:
                        ...
                        print('times7')
                        # np.save(MODEL_METHOD_DIR + 'real_adv_dec.npy', real_adv_dec)
                        print('times8')
            ...
            if is_hard:
                ...
                if method in ['center_ids', 'openmax']:
                    actual_test = data_test[:500, -1]
                else:
                    actual_test = data_test[:, -1]
