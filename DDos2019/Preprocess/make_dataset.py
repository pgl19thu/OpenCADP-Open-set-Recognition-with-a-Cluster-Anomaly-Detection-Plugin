...
import Preprocess.meta_data_2019 as md9
...
def make_subsets_by_num(file_path, num):
    ...
    for f in glob.glob(file_path + "*.csv"):
        ...
        td, td2 = cu.split_dataset_by_num(df, num)
        all_data.append(td)
    data = pd.concat(all_data, axis=0, ignore_index=True)
    return data
...
def convert_datatype(df, data_name):
    ...
    if data_name == "ids2017":
        ...
        for row in md7.FEATURE_LIST[2:-1]:
            df[row] = df[row].astype(float)
    if data_name == "ids2019":
        for row in md9.FEATURE_LIST[0:1]:
            df[row] = df[row].astype(float)
        for row in md9.FEATURE_LIST[2:-1]:
            df[row] = df[row].astype(float)
    return df
...
def main():
    file_path = "../all/"
    dst_path = "../percent/"
    df = make_subsets_by_num(file_path, 5000)
