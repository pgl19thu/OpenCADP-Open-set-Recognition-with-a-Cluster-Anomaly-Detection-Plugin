...
def plot_conmat(path, methods):
    ...
    known_classes= ['Bengin', 'FTP-BruteForce', 'SSH-Bruteforce', 'HOIC', 'LOIC-UDP', 'LOIC-HTTP', 'Bot', 'Unknwon']
    plot_conf_mat(re_best_results[0][2][15], path + '/images/' + methods + '_conf_cc.pdf', known_classes)
    plot_conf_mat(re_best_results[1][2][15], path + '/images/' + methods + '_conf_oc.pdf', known_classes)
    plot_conf_mat(re_best_results[2][2][15], path + '/images/' + methods + '_conf_oc_plugin.pdf', known_classes)
...
for num in range(1):
    for time in range(1):
        num=1
        time=1
        save_path = 'D:/ids2018/model/times_' + str(time) + '/' + 'unum' + str(num + 1) + '/'
        ...
        for method in methods:
            try:
                ...
                # re_best = np.load(save_path + method + '/re_results.npy', allow_pickle=True)
                # adv
                # plot_figs(save_path, thresholds, method,'real_adv_dec.txt.pkl','re_results.npy')
